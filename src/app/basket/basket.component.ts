...
export class BasketComponent implements OnInit, AfterViewInit {
  ...
  private _complexSearchBasket: ComplexSearchResult;
  private _allInteractorsInComplexSearchBasket: Interactor[] = [];
  ...
  ngOnInit() {
    ...
    this._allInteractorsInComplexSearchBasket = [];
    this._complexes = [];
    this.requestComplexesForNavigator();
    console.log(this._complexes);
    console.log(this._allInteractorsInComplexSearchBasket);
  }
  ...
  private requestComplexesForNavigator() {
    this.complexPortalService.findComplex(this._query).subscribe(complexSearch => {
      this._complexSearchBasket = complexSearch;
      if (this._complexSearchBasket.totalNumberOfResults !== 0) {
        for (let i = 0; i < complexSearch.elements.length; i++) {
          this._complexes.push(complexSearch.elements[i]);
          for (const component of complexSearch.elements[i].interactors) {
            if (!this._allInteractorsInComplexSearchBasket.some(interactor => interactor.identifier === component.identifier)) {
              this._allInteractorsInComplexSearchBasket.push(
                new Interactor(
